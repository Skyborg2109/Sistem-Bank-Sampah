...
<div class="animate-slide-in">
    ...
    <div class="mb-6 flex items-center justify-between">
        ...
        <div class="flex items-center space-x-2 text-sm text-gray-600">
            ...
            <span>Total: <strong>{{ $laporan->count() }}</strong> laporan</span>
        </div>
    </div>
                        ...
                        <tr class="table-row">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10 bg-gradient-to-br from-green-500 to-emerald-600 rounded-full flex items-center justify-center text-white font-bold text-sm">
                                        {{ strtoupper(substr($item->nama_warga, 0, 2)) }}
                                    </div>
                                    <div class="ml-4">
                                        <div class="text-sm font-semibold text-gray-900">{{ $item->nama_warga }}</div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <span class="text-sm font-medium text-gray-900">{{ $item->jenis_sampah }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                <div class="flex items-center justify-center space-x-2">
                                    {{-- View Button --}}
                                    <button 
                                        onclick="showDetail('{{ $item->nama_warga }}', '{{ $item->jenis_sampah }}', '{{ $item->berat }}', '{{ \Carbon\Carbon::parse($item->tanggal)->format('d M Y') }}', '{{ $item->keterangan }}')"
                                    ...
                                    {{-- Delete Button --}}
                                    <form action="/Admin/laporan/delete/{{ $item->id }}" 
                                          method="POST" 
                                          class="inline-block"
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-6">
                    <div class="flex items-center space-x-2">
                        ...
                        <span class="text-sm text-gray-600">Total Laporan: <span class="font-semibold text-gray-900">{{ $laporan->count() }}</span></span>
                    </div>
                    <div class="flex items-center space-x-2">
                        <div class="w-2 h-2 bg-indigo-500 rounded-full"></div>
                        <span class="text-sm text-gray-600">Total Berat: <span class="font-semibold text-gray-900">{{ number_format($laporan->sum('berat'), 2) }} Kg</span></span>
                    </div>
                </div>
